...
from scraper.spiders.paca import PACASpider
...
class DiscloseDREALPACAScraper(AddOn):
    ...
    def check_permissions(self):
        ...
        try:
            ...
            subject = f"DREAL PACA Scraper - Error connecting to DocumentCloud"
            content = f"Error:\n {e.__traceback__}"
    ...
    def main(self):
        ...
        self.client.session.headers.update(
            {"User-Agent": "Disclose DREAL PACA Scraper Add-On"}
        )
        ...
        self.set_message(
            f"Scraping DREAL PACA documents {str(self.target_year)} [{self.run_name}]"
        )
        process.start()
